                        ...
                        {loading ? (
                            ...
                            <>
                                ...
                                <form onSubmit={handleSubmit} className="space-y-5">
                                        ...
                                        <div key={index} className="bg-white/15 backdrop-blur-sm rounded-2xl p-6 space-y-4 border border-white/30 shadow-lg hover:shadow-xl hover:bg-white/20 transition-all animate-fadeIn" style={{ animationDelay: `${index * 0.1}s` }}>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div>
                                                    ...
                                                    <select value={skill.category} onChange={(e) => updateSkillEntry(index, 'category', e.target.value)} className="w-full px-4 py-3 border border-white/40 bg-white/30 backdrop-blur-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-[#99D6EA]/50 focus:border-white/60 transition-all">
                                                ...
                                                <div>
                                                    ...
                                                    <select value={skill.skillName} onChange={(e) => updateSkillEntry(index, 'skillName', e.target.value)} className="w-full px-4 py-3 border border-white/40 bg-white/30 backdrop-blur-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-[#99D6EA]/50 focus:border-white/60 transition-all">
                                            ...
                                            <div>
                                                ...
                                                <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                                                    {WILLINGNESS_OPTIONS.map((opt) => (
                                                        <button key={opt.value} type="button" onClick={() => updateSkillEntry(index, 'willingnessLevel', opt.value)} className={`px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${skill.willingnessLevel === opt.value ? 'bg-[#00245D] text-white shadow-lg shadow-[#00245D]/25' : 'bg-white/20 border border-white/40 text-[#00245D] hover:bg-white/30 hover:border-white/60 backdrop-blur-sm'}`} title={opt.description}>{opt.label}</button>
                                                    ))}
                                                </div>
                                            </div>
                                    ...
                                    <button type="button" onClick={addSkillEntry} className="w-full py-4 border-2 border-dashed border-white/30 rounded-2xl text-[#00245D]/60 hover:border-white/60 hover:text-[#00245D] hover:bg-white/10 transition-all font-medium backdrop-blur-sm">+ Add Another Skill</button>
